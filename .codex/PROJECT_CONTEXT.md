...
- 迁移前旧路径为 `stableye/zodiac-persona-kit`，当时的路径迁移提交 `9fc59f6` 与 Node.js 22 Actions run `30793663522` 已通过。当前公开 MIT 仓库为 `https://github.com/stableopen/zodiac-persona-kit`，默认分支 `main`，完整 Git 历史和既有 topics 保留；GitHub About homepage 指向当前仓库自身，尚未创建版本标签。
...
- 2026-08-17 产品复验返工：模式起手式改为只依赖合法 `modeId`，不再受历史消息数量限制；直接聊天仍仅在新会话显示旧快捷问题。浏览器以 2 条真实本地消息验证“旧会话→回首页→选安心→3 个起手式出现→点击只填框→继续编辑”，旧消息未清除且 console error=0。Hero 改为“先选对味表达，再用三种模式做事”；安心使用点显示“仅用于日常情绪整理，不提供心理诊断或治疗”。
