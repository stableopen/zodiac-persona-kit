...
1. 保留现有架构和 V0.1/V0.2 路径；由所有者查看私有生产站并决定公开访问时点。
2. 启用生产聊天前设置稳定私盐、持久 KV、模型配置和费用硬限制，并验证可信代理 IP 头与 Worker origin 覆盖。
3. 在真实平台验证双限额和互动分享二维码；安装并登录 GitHub CLI 后运行远程 Node 22 CI。
